...
def fetch(root, generations):
    ...
    for i in range(len(result['results']['bindings'])):
        ...
        for j in range(generations):
            urls.append(result['results']['bindings'][i]['person' + str(j)]['value'])
            names.append(result['results']['bindings'][i]['person' + str(j) + 'Label']['value'])
...
def getString(root, generations):
    ...
    for i in range(1, generations):
        lines += "?person" + str(i - 1) + " (wdt:P22|wdt:P25|wdt:P40) ?person" + str(i) + ". "

    return "SELECT " + persons + " WHERE {wd:"+ root +" (wdt:P22|wdt:P25|wdt:P40) ?person0." + lines + " SERVICE wikibase:label { bd:serviceParam wikibase:language '[AUTO_LANGUAGE],en'. } }"
